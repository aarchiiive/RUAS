...
# test_low_data_names = 'LOD/RGB_Dark/*'
# test_low_data_names = 'DarkFace_Train_2021/image/*'
test_low_data_names = 'Exdark/JPEGImages/IMGS_dark/*'
...
def main():
    ...
    model = Network()
    model = model.cuda()
    # model_path = Path('RUAS_LOD/weights.pt')
    # model_path = Path('RUAS_DarkFace/weights.pt')
    model_path = Path('RUAS_Exdark/weights.pt')
    # model_dict = torch.load('./ckpt/'+args.model+'.pt')
    model_dict = torch.load(model_path, map_location='cpu')
    model.load_state_dict(model_dict)

    # save_dir = Path('LOD/RGB_RUAS')
    # save_dir = Path('DarkFace_Train_2021/RUAS')
    save_dir = Path('Exdark/JPEGImages/IMGS_RUAS')
    ...
    with torch.no_grad():
        for _, (input, image_name) in enumerate(test_queue):
            ...
            save_images(u_list[-1], save_path)
