...
class Network(nn.Module):
    ...
    def forward(self, input):
        ...
        u_d, noise = self.denoise_net(u_list[-1].detach())
        u_list.append(u_d)
        return u_list, t_list
    ...
    def _denoise_loss(self, input, target):
        u_list, t_listt = self(input)
        denoise_loss = self._denoise_criterion(u_list[-1], u_list[-2].detach())
        return denoise_loss
...
class SmoothLoss(nn.Module):
    ...
    def rgb2yCbCr(self, input_im):
        ...
        B = input_im.size(0)
        out = temp.view(B, 3, input_im.shape[2], input_im.shape[3])
        return out
